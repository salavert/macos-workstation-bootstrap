...
The public repository intentionally contains no workstation secrets, corporate configuration, credentials, private repository inventory, or personal Git configuration. Its job is only to establish the minimum trusted tooling, authenticate GitHub interactively, obtain the private source of truth, validate it, and hand control to its bootstrap.
...
- an existing authenticated GitHub session is reused;
...
CI runs these checks on macOS. The behavioral tests use fake `brew`, `gh`, `git`, and `make` commands and an isolated `HOME`; they verify read-only modes, destination safety, resumability, and repeated installation without unintended state drift.
